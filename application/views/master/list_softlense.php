<div class="page-title text-right">
	<h4>Master Data <i class="ti-angle-right"></i> Master Softlense</h4>
</div>
<div class="container">
	<div class="card">
		<div class="card-block text-dark">
			<h4 class="card-title">Master Softlense</h4>
			<div class="card-body">
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Tambah Master Softlense</h5>
...
<div class="modal fade" id="modal_edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Update Master Softlense</h5>
				<button type="button" class="close keluar_" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
...
<script>
	...
	$(document).on('click', '.hapus', function() {
		...
		if(conf == true) {
			$.post("<?= site_url('master/master_data/hapus_jenis_barang/softlense/') ?>"+id, '', function(d) {
				if(d.s == 'sukses') {
					alert(d.m);
					location.reload();
				} else {
	...
	$(document).on('click', '.edit', function() {
		...
		$.post("<?= site_url('master/master_data/get_data_jenis/softlense/') ?>"+id, '', function(d) {
			if(d.s == 'sukses') {
	...
	$('#xyy').on('submit', function(e) {
		...
		$.ajax({
			url: "<?= site_url('master/master_data/update_jenis_barang/softlense') ?>",
			type: "POST",
			data:  $("#xyy").serialize(),
			cache: false,
			dataType: 'json',
	...
	$('#xyz').on('submit', function(e) {
		...
		$.ajax({
			url: "<?= site_url('master/master_data/simpan_jenis_barang/softlense') ?>",
			type: "POST",
			data:  $("#xyz").serialize(),
			cache: false,
			dataType: 'json',
